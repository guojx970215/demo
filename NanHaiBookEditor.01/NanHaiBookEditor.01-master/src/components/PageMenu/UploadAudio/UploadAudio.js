...
import { Select,Form,Input, Modal, Checkbox } from 'antd'
import Tags from '../../AudioMenu/components/Tags'
const { Option } = Select
class UploadAudio extends React.Component {
  ...
  handleOk = e => {
    ...
    const { link, config, isAddMusic, uploadFile } = this.state;
    let html = '';
    let dataExtra = {
  ...
  render () {
    const {config, link, visibleList, isAddMusic, addMusicDisable} = this.state
    ...
    return (
      <Modal
        ...
        <div style={{padding: '10px 0'}}>
          ...
          <Checkbox checked={isAddMusic} disabled={addMusicDisable} onChange={(e)=>{
            this.setState({
